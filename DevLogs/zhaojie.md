...
* 最大的问题就是这个播放器没有按需加载的功能。

#### 12.23 ####

##### 结果 #####
* 优化了之前的一些效果。
* 用一种极其丑陋的方式解决了播放器添加歌单时显示歌词的问题。

##### 问题及解决 #####
* 突然发现登录之后不能正确的获取到用户信息了，定位到问题了，因为现在后端部署在服务器上，改到本机就可以，因为登录后认证的ip是后端的ip，所以没有通过认证，很迷惑。
* 发现当歌曲是歌单最后一个时，会报错，原因是最后一首歌的下一个索引是0，减掉1后变成负数，作为数组下标报错，将其加上数组长度后再模数组长度解决。
* 遇到一个问题，需要移除三方库一个元素的onclick事件，但是这个事件注册时的函数是匿名函数，怎么做。
* 突然发现一个重大bug，去到详情页时如果输入的id不存在，会报错，而不是跳转到404页面。
* antd的message组件的全局配置好像没有生效，那还是在用到的地方局部配置。
* 用react-router的useNavigator钩子，但是和useEffect一起使用的，会被执行多次，最后改用window.location.href。
...
##### 问题及解决 #####
* 之前那个window.location.href的问题，修改了一下，因为这种方式的跳转太慢，我仔细审查了一下，之前会执行多次是因为用的事件监听，可以监听多个事件，而react的fiber机制导致其被执行了多次，就加了好几次事件监听，所以会跳转多次，改为事件绑定，就不会有这个问题，不过又出现了一个新问题，ts报错，原因是document.getElementsByClassname在ts是Element类型，没有onclick这个属性，得把它定义为HTMLElement，这样就可以了。
* 之前那个路由的问题，本来是在想如果后端返回404，我就跳转到404页面，可是那个SB接口返回的是200，还在想办法。
* 又发现一个bug，随机播放时会出现问题，原因是nextIndex其实不是指这首歌在列表里的下一个，但是这个播放器并没给出当前播放歌曲的索引怎么拿，暂时没想到办法解决。
* 找到了当前的索引，解决了这个问题。
* 发现很多地方健壮性不是很好，确实是没有经验了，不知道其实后端返回的数据结构不是固定的，会有一些字段没有，整个项目不太好改，当成是失败的经验了。
* 部署后路由出现问题，刷新会导致404，原因是这是单页面应用，这些路由的页面对nginx来说都不存在，配置一下nginx就可以了。
* 部署后获取音乐url出现问题，网上查了一下说是chrome新版的跨域策略，需要配置浏览器的配置，显然不合理，据两位导师反馈，在mac上没有这个问题。
* 部署后登录功能出现问题，不能存cookie，查了一下又是chrome的samesite问题。
* 将chrome更新后，播放问题得到解决，但是登录问题没有解决。
